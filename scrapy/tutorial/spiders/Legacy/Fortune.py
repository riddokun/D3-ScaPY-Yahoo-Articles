...
class Fortune(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    fname = "C:\lol\Fortune-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-fortune/"]

    # In this class we will get the basic data and fill in the information for them
    def __init__(self):
    ...
    def parse(self, response):
        ...
        for articles in arry_hxs:
            ...
            #split string by month day year and others
                ...
                url = ""
                url = ''.join(item['Link'])
                req = Request(url, callback=self.parse_Fortune, dont_filter=True, priority=100)  # print url
    ...
    def parse_Fortune(self, response):  # data stored
        global writer
        ...
        Fortune.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
