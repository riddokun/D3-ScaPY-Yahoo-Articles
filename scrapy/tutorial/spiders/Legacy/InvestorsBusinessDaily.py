...
class InvestorsBusinessDaily(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    yname = "C:\lol\InvestorsBusinessDaily-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-investors-business-daily/"]
    ...
    def parse(self, response):
        ...
        for articles in arry_hxs:
            ...
            #split string by month day year and others
                ...
                if "@" in str1:
                    url = ''.join(item['Link'])
                    req = Request(url, callback=self.parse_daily, dont_filter=True, priority=100)  # print url
                else:
    ...
    def parse_yahoo(self, response):  # data stored
        ...
        InvestorsBusinessDaily.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
                                  articletime.encode("utf-8"), article.encode("utf-8"), link.encode("utf-8")])

    def parse_daily(self, response):  # data stored
        global writer
        sel = HtmlXPathSelector(response)
        article = ''.join(sel.xpath('//p/text()').extract())
        subheadline = " "
        articletime = ''.join(title.xpath('//span[@itemprop="printEdition"]/text()').extract())
        thetime = ''.join(title.xpath('//span[@itemprop="datePublished"]/text()').extract())
        print articletime
        print thetime
