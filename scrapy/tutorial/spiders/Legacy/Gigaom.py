...
class Gigaom(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    fname = "C:\lol\Gigaom-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-paidcontent/"]
    ...
    def parse(self, response):
        ...
        for articles in arry_hxs:
            ...
            #split string by month day year and others
                ...
                if "@" in str:
                    url = ''.join(item['Link'])
                    req = Request(url, callback=self.parse_Gigaom, dont_filter=True, priority=100)  # print url
                else:
    ...
    def parse_Gigaom(self, response):  # data stored
        global writer
        ...
        Gigaom.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
