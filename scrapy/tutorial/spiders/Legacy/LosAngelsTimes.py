...
class LosAngelsTimes(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    fname = "C:\lol\LostAngelsTime-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-los-angeles-times/"]
    ...
    def parse(self, response):
        ...
        self.driver.get("http://finance.yahoo.com/news/provider-los-angeles-times/")
        ...
        for articles in arry_hxs:
            ...
            #split string by month day year and others
                ...
                url = ""
                url = ''.join(item['Link'])
                req = Request(url, callback=self.parse_LATimes, dont_filter=True, priority=100)  # print url
    ...
    def parse_LATimes(self, response):  # data stored
        global writer
        ...
        LosAngelsTimes.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
