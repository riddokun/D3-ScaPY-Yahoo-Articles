...
class Engadget(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    fname = "C:\lol\Engadget-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-engadget/"]
    ...
    def parse(self, response):
        ...
        self.driver.get("http://finance.yahoo.com/news/provider-engadget/")
        ...
        for articles in arry_hxs:
            ...
            #split string by month day year and others
                ...
                url = ""
                url = ''.join(item['Link'])
                req = Request(url, callback=self.parse_engadget, dont_filter=True, priority=100)  # print url
    ...
    def parse_engadget(self, response):  # data stored
        global writer
        ...
        Engadget.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
