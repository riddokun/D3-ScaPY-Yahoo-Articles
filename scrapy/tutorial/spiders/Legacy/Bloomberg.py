...
class Bloomberg(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    bname = "C:\lol\Bloom-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-bloomberg/"]
    ...
    def parse_bloomberg(self, response):  # data stored
        global writer
        ...
        Bloomberg.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
    ...
    def parse_yahoo(self, response):  # data stored
        ...
        article = ''.join(sel.xpath('//section[@class="article-body"]/text()').extract())
