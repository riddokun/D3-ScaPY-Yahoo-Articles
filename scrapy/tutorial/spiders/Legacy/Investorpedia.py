...
class Investopedia(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    xname = "C:\lol\Investopedia-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-investopedia/"]
    ...
    def parse(self, response):
        ...
        for articles in arry_hxs:
            ...
            #split string by month day year and others
                ...
                url = ""
                url = ''.join(item['Link'])
                req = Request(url, callback=self.parse_Investopedia, dont_filter=True, priority=100)  # print url
    ...
    def parse_Investopedia(self, response):  # data stored
        global writer
        sel = HtmlXPathSelector(response)
        article = ''.join(sel.xpath('//div[@class="content-box"]/p/text()').extract())
        ...
        Investopedia.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
