...
class CNBC(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    fname = "C:\lol\CNBC-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-cnbc/"]
    ...
    def parse(self, response):
        ...
        for articles in arry_hxs:
            ...
            #split string by month day year and others
                ...
                if "@" in str1:
                    url = ''.join(item['Link'])
                    req = Request(url, callback=self.parse_CNBC, dont_filter=True, priority=100)  # print url
                else:
    ...
    def parse_CNBC(self, response):  # data stored
        global writer
        ...
        CNBC.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
