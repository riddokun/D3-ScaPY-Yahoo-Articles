...
class MoneyTalk(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    aaname = "C:\lol\MoneyTalk-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-moneytalksnews/"]
    ...
    def parse_page2(self, response):  # data stored
        ...
        MoneyTalk.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
