...
class CreditCards(CrawlSpider):
    global last_title
    current_time = datetime.datetime.now().strftime("%Y-%m-%d")
    current_hour = time.strftime("%X").split(":")[0]
    print current_hour
    oname = "C:\lol\CreditCards-" + current_time + ".csv"
    ...
    start_urls = ["http://finance.yahoo.com/news/provider-credit-cards/"]
    ...
    def parse_page2(self, response):  # data stored
        ...
        CreditCards.writer.writerow([title.encode("utf-8"), subheadline.encode("utf-8"), source.encode("utf-8"), linktime.encode("utf-8"),
